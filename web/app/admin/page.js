            ...
            <div className="bg-red-950/20 border border-red-800/40 rounded-2xl p-6 space-y-4 mt-4">
              ...
              {!otpVerified ? (
                ...
                <div className="space-y-3">
                  ...
                  {/* USDC Supervised Pilot Kill Switch (SI 99 of 2026) */}
                  <div className="border border-amber-700/40 rounded-lg p-4 bg-amber-900/10">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-sm font-semibold text-amber-300">USDC Supervised Pilot</p>
                        <p className="text-xs text-amber-400/70 mt-0.5">Statutory Instrument 99 of 2026 — RBZ supervised mode</p>
                        <p className="text-xs text-gray-500 mt-1">
                          Current: <span className={settings['usdc_pilot_enabled']?.value === 'true' ? 'text-green-400 font-bold' : 'text-red-400 font-bold'}>
                            {settings['usdc_pilot_enabled']?.value === 'true' ? 'ENABLED' : 'DISABLED'}
                          </span>
                        </p>
                        {settings['usdc_pilot_enabled']?.value !== 'true' && (
                          <p className="text-xs text-red-300/70 mt-1">⚠ Do NOT enable until formal RBZ written confirmation is received</p>
                        )}
                      </div>
                      <button onClick={async()=>{
                        const current = settings['usdc_pilot_enabled']?.value === 'true';
                        const newVal  = (!current).toString();
                        const label   = current ? 'DISABLE' : 'ENABLE';
                        if(!confirm(`${label} the USDC supervised pilot? ${!current ? 'Only proceed if you have formal RBZ written confirmation.' : ''}`)) return;
                        const t=localStorage.getItem('token');
                        const r=await fetch(`${API}/super-admin/sensitive-setting`,{method:'PUT',headers:{Authorization:`Bearer ${t}`,'Content-Type':'application/json'},body:JSON.stringify({key:'usdc_pilot_enabled',value:newVal,otpCode})});
                        const d=await r.json();
                        if(r.ok){notify('success',`USDC pilot ${label}D`);fetchSettings();}
                        else{notify('error',d.error);setOtpVerified(false);setOtpSent(false);setOtpCode('');}
                      }} className={`px-4 py-2 rounded-lg text-xs font-semibold whitespace-nowrap ${settings['usdc_pilot_enabled']?.value === 'true' ? 'bg-red-700 hover:bg-red-600 text-white' : 'bg-green-700 hover:bg-green-600 text-white'}`}>
                        {settings['usdc_pilot_enabled']?.value === 'true' ? 'Disable Pilot' : 'Enable Pilot'}
                      </button>
                    </div>
                  </div>
                  {[
